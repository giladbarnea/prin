...
def fetch_html(urls_to_try: List[str], client: httpx.Client) -> Optional[Tuple[str, str]]:
...
def crawl(seed_url: str, json_path: str) -> None:
    ...
    try:
        while to_visit:
            ...
            urls_to_try = pick_fetch_urls(current, canon_to_sample.get(current), seed_scheme)
            ...
            for l in links:
                if not same_domain(l, allowed_domain):
                    continue
                c = canonical_key(l)
                if not c:
                    continue
