...
from tests.utils import count_opening_xml_tags, count_md_headers
...
def test_no_docs_excludes_markdown_and_rst(fs_root):
    ...
    assert "readme.md" not in out and "README.md" not in out and "notes.rst" not in out
...
def test_extension_filters_by_extension(fs_root):
    ...
    assert "<foo.py>" in out and "<src/app.py>" in out
...
def test_exclude_glob_and_literal(fs_root):
    ...
    assert "<src/app.py>" not in out and "<src/util.py>" not in out
    assert "readme.md" not in out and "README.md" not in out
