...
#### Members
- Protocol: `src/prin/core.py`: `SourceAdapter` with `resolve`, `list_dir`, `read_file_bytes`, `is_empty` (and `Entry`/`NodeKind` shapes).
...
- `resolve` returns a stable POSIX-like root for display anchoring.
